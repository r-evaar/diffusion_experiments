...
def initialize(code=True) -> str:
    ...
    Args:
        code: A boolean for whether to return the device name as a string.
...
class ClassTrainer:
    ...
    def verbose_step(self, update, *args):
        ...
        if update == 'epoch_start':
            ...
            loss, acc, t0 = args  # validation loss, validation accuracy
            print(f'| val: l={loss:.2f}, a={acc:06.2f}% [{tic()-t0:.2f}s]', end=' ')

        elif update == 'lr':
            print(f'| lr={self.scheduler.get_last_lr()[0]:f}')  # Last learning rate

    def progress_step(self, tag, loss, acc, t=None):
        ...
        self.progress[tag]['time'].append((tic()-t) if t is not None else None)
    ...
    def run_epoch(self, loader):
        ...
        for i, (x, y) in enumerate(loader):
            ...
            self.progress_step('batch', loss, acc, t1)

        self.verbose_step('train', e_loss, e_acc, b_avg_time, e_time_0)
        self.progress_step('train', e_loss, e_acc, e_time_0)
    ...
    def val_epoch(self, loader):
        ...
        self.verbose_step('val', loss, acc, v_time_0)
        self.progress_step('val', loss, acc, v_time_0)
